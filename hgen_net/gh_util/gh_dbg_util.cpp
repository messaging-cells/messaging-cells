...
std::string 
gh_get_stack_trace( const std::string & file, int line ){
...
void
gh_ptr_call_stack_trace(FILE* out_fp) {
	if(out_fp == gh_null){
		out_fp = stderr;
	}

	void* trace[GH_MAX_CALL_STACK_SZ];
	memset((uint8_t*)trace, 0, GH_MAX_CALL_STACK_SZ * sizeof(void*));

	size_t trace_sz = backtrace(trace, GH_MAX_CALL_STACK_SZ);

	fprintf(out_fp, "trace_size=" GH_PRT_SIZE_T " \n", trace_sz);

	char **stack_strings = backtrace_symbols(trace, trace_sz);
	fprintf(out_fp, "STACK_STRACE------------------------------\n");
	for( size_t ii = 1; ii < trace_sz; ii++ ) {
		if(ii >= GH_MAX_CALL_STACK_SZ){ break; }
		fprintf(out_fp, "%s \n", stack_strings[ii]);
	}
...
bool 
gh_call_assert(char* out_fnam, bool is_assert, bool prt_stck, bool cond, 
		const char* fnam, int line, const char* ck_str, const char* fmt, ...)
{
	...
	if(do_prt){
		...
		if(is_assert && (ck_str != gh_null)){
			fprintf(out_file, "ASSERT '%s' FAILED.\n", ck_str);
		} 
		if(prt_stck){
			gh_ptr_call_stack_trace(out_file);
		}

		if(fmt != NULL){
			char prt_buff[GH_MAX_STR_SZ];
			va_list ap;

			va_start(ap, fmt);
			int size = vsnprintf(prt_buff, GH_MAX_STR_SZ, fmt, ap);
			va_end(ap);

			prt_buff[GH_MAX_STR_SZ - 1] = '\0';
	...
	if((out_fnam != gh_null) && (out_file != stderr)){
		fclose(out_file);
	}
