...
long	
to_interval(long val, long min, long max) mc_external_code_ram;
...
public:
	tak_mak(unsigned long ini_val = 0) mc_external_code_ram;

	tak_mak(const unsigned long* init_key, int key_length) mc_external_code_ram;

	void init_tak_mak() mc_external_code_ram;

	void init_with_long(unsigned long s) mc_external_code_ram;
	void init_with_array(const unsigned long* init_key, int key_length) mc_external_code_ram;

	unsigned long gen_rand_int32(void) mc_external_code_ram;

	long gen_rand_int32_ie(long min, long max) mc_external_code_ram;
};
