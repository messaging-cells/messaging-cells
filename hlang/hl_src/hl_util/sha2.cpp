...
void sha2_update( sha2_context *ctx, unsigned char *input, int ilen )
{
    ...
    if( left && ilen >= fill )
    {
        hl_memcpy( (void *) (ctx->buffer + left),
                (void *) input, fill );
           ...
           unsigned char *output, int is224 )
{
    ...
    hl_memset( &ctx, 0, sizeof( sha2_context ) );
}
                       ...
                       unsigned char *key, int keylen )
{
    ...
    hl_memset( ctx->ipad, 0x36, 64 );
    hl_memset( ctx->opad, 0x5C, 64 );
...
void sha2_hmac_finish( sha2_context *ctx, unsigned char *output )
{
    ...
    hl_memset( tmpbuf, 0, sizeof( tmpbuf ) );
}
